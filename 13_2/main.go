...
func main() {
	...
	for {
		...
		// newcarts := carts[:0]
		for i := range carts {
			if _, ok := collidedIDs[i]; ok {
				fmt.Printf("skip collided car #%d\n", i)
				continue
			}
			carts[i].cartMove()
			terrain := tracksMap[carts[i].y][carts[i].x]
			carts[i].cartPostMove(terrain)
			collided, collidedID := carts.checkCollision(i)
			if collided {
				fmt.Printf("Collided[%d]: %d:%d\n", collidedID, carts[i].x, carts[i].y)
				// remove both carts
				collidedIDs[i] = true
				collidedIDs[collidedID] = true
				// return
			}
		}
...
func (s carts) checkCollision(cartID int) (collided bool, collidedID int) {
	collided = false
	for i := range s {
		if i == cartID {
			continue
